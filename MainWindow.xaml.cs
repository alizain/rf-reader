...
namespace RfReader_demo
{
    ...
    public partial class MainWindow : Window
    {
        ...
        void readXML(bool starting)
        {
            ...
            //string fileName = System.IO.Path.Combine(Directory.GetParent(System.IO.Directory.GetCurrentDirectory()).Parent.FullName + "\\Credential", "appData.xml");
        ...
        void SaveXML(string ipAddress, string portNumber, string serverName, string databaseName, string username, string password, List<Devices> deviceData)
        {
            //string path = Directory.GetParent(System.Reflection.Assembly.GetExecutingAssembly().Location).FullName;
            //string fileName = System.IO.Path.Combine(Directory.GetParent(System.IO.Directory.GetCurrentDirectory()).Parent.FullName + "\\Credential", "appData.xml");
            //string fileName = System.IO.Path.Combine(path, "appData.xml");
            ...
            using (SentrySdk.Init(conn))
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        SentrySdk.CaptureMessage("Your Port Name or Table Name not found.");
                        return false;
                    }
                }
                catch (Exception ex)
        ...
        private void btnLive_Click(object sender, RoutedEventArgs e)
        {
            using (SentrySdk.Init(conn))
            {
                try
                {
                    ...
                    var connectionTest = ConnectionTest(false);
        ...
        {
            var status = ConnectionTest(true);
        }
        ...
        private void btn_DbConfig_Click(object sender, RoutedEventArgs e)
        {
            using (SentrySdk.Init(conn))
            {
                try
                {
                    ...
                    //string path = System.IO.Path.Combine(Directory.GetParent(System.IO.Directory.GetCurrentDirectory()).Parent.FullName + "\\Credential", "appData.xml");
            ...
            using (SentrySdk.Init(conn))
            {
                try
                {
                    ...
                    if (selectedItem != null)
                    {
                        ...
                        SaveXML(ipAddress, portNumber, serverName, databaseName, username, password, deviceData);
                        CompareXML();
                ...
                if (messageBoxResult == MessageBoxResult.Yes)
                {
                    ...
                    if (deviceData != null)
                    {
                        ...
                        else
                        {
                            ...
                            SentrySdk.CaptureMessage("Please enter Table Name and Device Port in Device Configuration.");
                        }
                    }
                    else
                    {
